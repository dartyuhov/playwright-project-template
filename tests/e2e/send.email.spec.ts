...
} from '../../src/mocks/send.email.mock';
...
test.describe('Send email', () => {
  test('user should be able to send email', async ({ portfolio, page }) => {
    ...
    await expect(notification.description.first()).toHaveText(
      'Your mail has been sent! I will reach you out you as soon as possible!',
    );
  });

  test('should see error if emailJs reterns error', async ({
    portfolio,
    page,
  }) => {
    ...
    await expect(notification.description.first()).toHaveText(
      `Oops, something went wrong.You can contact me directly at ${userData.email}`,
    );
  });
});
